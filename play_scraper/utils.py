...
from play_scraper import settings, i18n

log = logging.getLogger(__name__)


def default_headers() -> Dict[AnyStr, AnyStr]:
    return {
...
def clean_titles(title: AnyStr) -> AnyStr:
    return re.sub("\\xa0", " ", re.sub("[(\\u200d)(\\u200c)]", "", title))


def generate_post_data(
    results=None, page=None, pagtok=None, children=0
) -> Dict[AnyStr, AnyStr]:
    """
...
def build_url(method, query) -> AnyStr:
    """Creates the absolute url for a type of object. E.g. details, developer,
    or similar.

    :param query: ID of a play store entity (developer, app etc.)
    ...
    url = "{base}/{method}?id={id}".format(
        base=settings.BASE_URL, method=method, id=query
    )
    return url


def build_collection_url(category: AnyStr = "", collection: AnyStr = ""):
    ...
    url = "{base}{category}{collection}".format(
        base=settings.BASE_URL, category=category, collection=collection
    )
...
def send_request(
    ...
    :param verify: Flag for requesting SSL verification.
...
def parse_additional_info(soup: BeautifulSoup, language: AnyStr = "en"):
    """Parses an app's additional information section on its detail page.

    :param language: The language of play store page
    ...
    for title_div in section_titles_divs:
        ...
        if section_title in title_normalization:
            ...
            if title_key == "content_rating":
                ...
                if value_div.text:
                    value = value_div.text.replace("\xa0", " ")
...
def parse_screenshot_src(img: Tag) -> AnyStr:
    """
...
def parse_app_details(soup: BeautifulSoup):
    ...
    try:
        ...
        num_ratings = [
            re.search(r"\d+\%", rating.attrs.get("style")).group(0)
...
def extract_id_query(url: AnyStr):
    """
    ...
    if not absolute_url.startswith("https"):
        absolute_url = urljoin(settings.BASE_URL, url)
...
def parse_card_info(soup: BeautifulSoup) -> Dict[AnyStr, Any]:
    ...
    url = urljoin(
        settings.BASE_URL, soup.select_one("a.card-click-target").attrs["href"]
    )
    icon = urljoin(
        settings.BASE_URL, soup.select_one("img.cover-image").attrs["src"].split("=")[0]
    )
...
def parse_app_details_response_hook(response: Response, *args, **kwargs):
    """
...
def multi_futures_app_request(
    ...
    workers: int = settings.CONCURRENT_REQUESTS,
):
    """
    :param app_ids: A list of app IDs.
    :param headers: A dictionary of custom headers to use.
    :param verify: Flag for requesting SSL verification.
    :param workers: Number of concurrent request workers
